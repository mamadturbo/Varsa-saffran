    ...
    <section className="relative h-screen min-h-[640px] w-full overflow-hidden">
      ...
      {HERO_SLIDES.map((slide, i) => (
        ...
        >
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-t from-ink-950 via-ink-950/55 to-ink-900/40" />
          <div className="absolute inset-0 bg-gradient-to-l from-ink-950/70 via-transparent to-ink-950/30" />
        </div>
      ))}
      ...
      <div className="relative z-10 flex h-full items-center">
        <div className="container-luxe">
          <div className="max-w-3xl">
            ...
            <h1
              key={`title-${index}`}
              className="mt-8 animate-fade-up font-display text-4xl font-bold leading-[1.3] text-cream-50 text-balance sm:text-5xl lg:text-6xl"
            >
            ...
            <p
              key={`sub-${index}`}
              className="mt-8 max-w-xl animate-fade-up text-lg leading-9 text-cream-200 text-pretty [animation-delay:0.15s]"
            >
        ...
        {HERO_SLIDES.map((_, i) => (
          <button
            ...
            className={`h-1.5 rounded-full transition-all duration-500 ease-luxe ${
              i === index ? 'w-8 bg-gold-400' : 'w-3 bg-cream-200/30 hover:bg-cream-200/50'
            }`}
